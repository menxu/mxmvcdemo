...
public class RedisDemo {
    ...
    private static void init() {
        ...
        jedisPool = new JedisPool(config, "176.122.157.64", 6379, 5000, "123456", 0);
    }
    ...
    private static void string() throws InterruptedException {
        ...
        jedis.set("string", "1234");
        System.out.println("从redis取string： " + jedis.get("string"));

        jedis.incr("string"); // 自增，不存在testInt则自增结果是1，如果不是字符串，自增会报JedisDataException
        System.out.println("从redis自增取string： " + jedis.get("string"));

        jedis.decr("string");
        System.out.println("从redis自减取string： " + jedis.get("string"));

        jedis.append("string", "56abc");
        System.out.println("从redis追加取string： " + jedis.get("string"));

        String subString = jedis.substr("string", 2, 6);
        System.out.println("substr方法的返回值：" + subString);
        System.out.println("从redis中获取切割后的string：" + jedis.get("string")); // 可以看出，substr方法并不会破坏原有值，只是取出来加工而已

        jedis.rename("string", "newString");
        System.out.println("string改名成newString后值为：" + jedis.get("newString") + ", string exists: " + jedis.exists("string"));

        String type = jedis.type("newString");
        System.out.println("newString的数据类型是：" + type);

        long length = jedis.strlen("newString");
        System.out.println("newString的字符串长度为：" + length);

        jedis.set("testString6", "mm");
        jedis.set("testString7", "hh");
        jedis.set("testString8", "dd");
        jedis.set("testString99", "aa");
        ...
        jedis.del("testString6"); // 字符串删除
        System.out.println("从redis删除testString6后，testString6是否还存在：" + jedis.exists("testString6"));

        jedis.set("testString2", "abcdefghi");
    ...
    private static void list() {
        ...
        // 列表的插入与获取(可以重复)
        jedis.lpush("testList", "a"); // 从左边插入
        jedis.lpush("testList", "b");
        jedis.lpush("testList", "c");
        jedis.lpush("testList", "d");
        jedis.lpush("testList", "e");
        jedis.lpush("testList", "f");
        jedis.rpush("testList", "g"); // 从右边插入
